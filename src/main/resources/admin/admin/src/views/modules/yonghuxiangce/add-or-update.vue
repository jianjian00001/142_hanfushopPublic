          ...
          <el-form-item class="input" label="姓名" prop="xingming">
              <el-input v-model="ruleForm.xingming"
                placeholder="姓名" readonly></el-input>
                                                                                                                                                                                                      ...
                                                                                                                                                                                                      <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="相册图片" prop="xiangcetupian">
                <editor
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.xiangcetupian"
                    class="editor"
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.xiangcetupian" label="相册图片" prop="xiangcetupian">
                    <span v-html="ruleForm.xiangcetupian"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
	...
	this.ruleForm.xiangcetupian = this.ruleForm.xiangcetupian.replace(reg,'../../../hanfushop/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `yonghuxiangce/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.yonghuxiangceCrossAddOrUpdateFlag = false;
                  this.parent.search();
